...
public class FriendFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener, ExpandableListView.OnGroupClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        if (view == null) {
            DialogViewBuilder.init(getActivity(), "正在加载好友列表......");
    ...
    public void getData() {
        if (!XmppTool.getInstance().isConnection()) {
            UIUtils.showShortToast(getActivity(), "已断开,正在重连中....");
            return;
        }
        new Thread() {
            ...
            public void run() {
                ...
                for (int i = 0; i < groups.size(); i++) {
                    ...
                    for (int j = 0; j < child.size(); j++) {
                        ...
                        User users = new Gson().fromJson(list.get(0).name, User.class);
                        int status = 6;
                        if (map.containsKey(users.user)) {
                            status = (int) map.get(users.user);
                        }
    ...
    class MyFriendExpadableAdapter extends BaseExpandableListAdapter {
        ...
        public View getChildView(int groupPosition, int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
            ...
            holder.tv_name.setText(xf.user.nickname);
            switch (xf.status) {
                //0.在线 1.Q我吧 2.忙碌 3.勿扰 4.离开 5.隐身 6.离线
                case 5:
                case 6:
                    holder.iv_statu.setVisibility(View.INVISIBLE);
                    holder.tv_statu.setTextColor(getResources().getColor(R.color.dark_gray));
            ...
            if (xf.user.icon.equals("")) {
                if (xf.user.sex.equals("男")) {
                    holder.cv_icon.setImageResource(R.mipmap.avatar_male);
                } else {
                    holder.cv_icon.setImageResource(R.mipmap.avatar_female);
                }
            } else {
                if (xf.user.icon.substring(0, 4).equals("http")) {
                    Picasso.with(getActivity()).load(xf.user.icon).resize(200, 200)
                            .placeholder(R.mipmap.qq_addfriend_search_friend)
                            .error(R.mipmap.qq_addfriend_search_friend)
                            .centerInside().into(holder.cv_icon);
                } else {
                    Picasso.with(getActivity()).load(Constant.URL.ICON + "?name=" + xf.user.icon)
